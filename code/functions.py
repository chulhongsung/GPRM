...
def find_tilde_a(z, gamma, beta, delta, quantile_dl):
    ...
    tilde_a =  tf.cast(tf.clip_by_value((z - gamma + tf.math.reduce_sum(mask_bl * mask_dl)) / (tf.math.reduce_sum(mask_bl)+0.00000001), clip_value_min=0, clip_value_max=1), dtype=tf.dtypes.float32)
    
    return tilde_a

def crps(z, gamma, beta, delta, quantile_dl):
    mask = z >= quantile_dl # l0를 찾기위한 mask
    ...
    crps = (1+tf.math.log(tilde_a))*(z-gamma) + tf.math.reduce_sum(bl*(((-1/2) * ((1-dl)**2)) + 1 - tf.maximum(tilde_a, dl) + dl * tf.maximum(tf.math.log(tilde_a), tf.math.log(dl))))
